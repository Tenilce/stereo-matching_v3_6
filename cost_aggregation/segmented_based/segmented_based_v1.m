...
% using moving averages for computing the matching costs.
 ...
 % Example
 % [D, C_min, C] = segmented_based_v1(I1,I2,0,15,12,12,'SSD',0.01);
...
function [D, C_min, C] = segmented_based_v1(I1, I2, min_d, max_d, h, w, method, lambda)

% Prepare image segmentation using unoptimized mean-shift algorithm
% Ms  : Mean Shift (color)
% Ms2 : Mean Shift (color + spatial)
bw = 0.2; % bandwidth
[Ims1, Nms1] = Ms2(I1,bw); % Ims (the segmented image), Nms (number of segments)
[Ims2, Nms2] = Ms2(I2,bw);
